...
namespace CTRPluginFramework {
    ...
    SpeedometerController::NumericSpeedometer::NumericSpeedometer(SpeedUnit unit) : Speedometer() {
        SetUnit(unit);
        vControl = new VisualControl("SpdNuControl");
    ...
    void SpeedometerController::NumericSpeedometer::Create(VisualControl::GameVisualControl* control, void* createArg) {
        ...
        own->rootHandle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("R_speed"), 0);
        own->number0Handle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("P_speed-00"), 0);
        own->number1Handle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("P_speed-01"), 0);
        own->number2Handle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("P_speed-02"), 0);
        own->number3Handle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("P_speed-03"), 0);

        u32 textHandle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("T_unit"), 0);
        const char* units[] = {"km/h", "mph"};
        VisualControl::MessageString str(units[(u32)own->GetUnit()]);
        layout->vtable->replaceMessageImpl(layout, textHandle, str, nullptr, nullptr);
    }
    ...
    SpeedometerController::GraphicalSpeedometer::GraphicalSpeedometer(SpeedUnit unit) : Speedometer() {
        SetUnit(unit);
        vControl = new VisualControl("SpdGrControl");
        vControl->SetUserData(this);
        vControl->SetDeallocateCallback(SpeedometerController::OnDeallocate);
        vControl->SetAnimationDefineCallback(DefineAnimation);
        vControl->SetOnCreateCallback(Create);
    ...
    void SpeedometerController::GraphicalSpeedometer::Create(VisualControl::GameVisualControl* control, void* createArg) {
        ...
        own->rootHandle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("R_speed"), 0);
        own->rotationHandle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("N_rot"), 0);
        ...
        if (own->GetUnit() == SpeedUnit::MPH) {
            ...
            const char* units[] = {"15", "30", "45", "60", "75", "90"};
            for (int i = 0; i < 6; i++) {
                textHandle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase(elements[i]), 0);
                str = VisualControl::MessageString(units[i]);
                layout->vtable->replaceMessageImpl(layout, textHandle, str, nullptr, nullptr);
            }
        }
    }
    ...
    SpeedometerController::DoubleDashSpeedometer::DoubleDashSpeedometer(SpeedUnit unit) : Speedometer() {
        SetUnit(unit);
        vControl = new VisualControl("SpdDDControl");
        vControl->SetUserData(this);
        vControl->SetDeallocateCallback(SpeedometerController::OnDeallocate);
        vControl->SetAnimationDefineCallback(DefineAnimation);
        vControl->SetOnCreateCallback(Create);
    ...
    void SpeedometerController::DoubleDashSpeedometer::Create(VisualControl::GameVisualControl* control, void* createArg) {
        ...
        own->rootHandle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("R_speed"), 0);
        own->number1Handle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("P_num_01"), 0);
        own->number2Handle = layout->vtable->getElementHandle(layout, MarioKartFramework::SafeStringBase("P_num_02"), 0);
    }
    ...
    void SpeedometerController::DoubleDashSpeedometer::Calc(VisualControl::GameVisualControl* control) {
        ...
        if (own->state == State::HIDDEN) {
            ...
            if (starTimer && (own->state == State::NORMAL || own->state == State::SPEED))
